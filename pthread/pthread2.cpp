...
void* thread_mul(void* arg) {
    ...
    LL* b = param->b;
    int n = param->n;
    int modIndex = param->modIndex;
    LL mod = M[modIndex];

    poly_multiply_part(a, b, C[modIndex], n, mod);
...
void* crt_thread(void* arg) {
    CRTParam* param = (CRTParam*)arg;
    for (int i = param->start; i < param->end; ++i) {
        param->ab[i] = crt_merge(i, param->P);
    }
    pthread_exit(nullptr);
}
const int NUM_CRT_THREADS=5;
void poly_multiply(LL* a, LL* b, LL* ab, int n,LL p_target) {
    pthread_t threads[MOD];
    ThreadParam params[MOD];

    for (int i = 0; i < MOD; ++i) {
        params[i] = { a, b, n, i };
        pthread_create(&threads[i], nullptr, thread_mul, &params[i]);
    }
   ...
   // 启动CRT合并线程
   pthread_t crt_threads[NUM_CRT_THREADS];
   CRTParam crt_params[NUM_CRT_THREADS];
   int m = 2 * n - 1;
   int chunk_size = m / NUM_CRT_THREADS;
   int remainder = m % NUM_CRT_THREADS;
   int start = 0;

   for (int t = 0; t < NUM_CRT_THREADS; ++t) {
       int end = start + chunk_size;
       if (t < remainder) end += 1;

       crt_params[t]={ab,start,end,p_target};
       pthread_create(&crt_threads[t], nullptr, crt_thread, &crt_params[t]);
       start = end;
   }
